...
class Solution:
    def carFleet(self, target, position, speed):
        ...
        for i in range(len(speed)):
            timeToArrive.append(((target-position[i])/speed[i],position[i]))
    
        timeToArrive.sort(key=lambda x:x[1], reverse=True)

        for i in range(len(speed)):
            if stack==[]:
                stack.append(timeToArrive[i])
                continue
            if timeToArrive[i][0]>stack[len(stack)-1][0]:
                stack.append(timeToArrive[i])
        return len(stack)
